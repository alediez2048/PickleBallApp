...
import { Game, SkillLevel, GameStatus, User, Location } from "@/types/games";

// Mock the API
jest.mock("@/services/api/games", () => {
  const mockApi = {
    getGames: jest.fn().mockImplementation(() => Promise.resolve([])),
    createGame: jest.fn().mockImplementation((game) =>
      Promise.resolve({
        ...game,
        id: "1",
        status: "Upcoming",
...
const TestComponent: React.FC = () => {
  ...
  return (
    <View>
      ...
      <Pressable testID='create-game' onPress={() => createGame(mockGameData)}>
