...
import { Game, GameStatus, SkillLevel } from "@/types/games";
...
const GamesBySkillLevelComponent: React.FC<GamesBySkillLevelProps> = ({
  skillLevel,
}) => {
...
const PaginatedGamesComponent: React.FC<PaginatedGamesProps> = ({
  page,
  pageSize,
}) => {
...
describe("Game Selectors", () => {
  ...
  describe("useGamesByStatus", () => {
    it("filters games by status", () => {
      const tree = renderer
        .create(<GamesByStatusComponent status={GameStatus.Upcoming} />)
        .toJSON();
      expect(tree).toMatchSnapshot();
    });
  });
  ...
  describe("useGamesBySkillLevel", () => {
    it("filters games by skill level", () => {
      const tree = renderer
        .create(<GamesBySkillLevelComponent skillLevel={SkillLevel.Beginner} />)
        .toJSON();
      expect(tree).toMatchSnapshot();
    });
  });
  ...
  describe("useFilteredGames", () => {
    it("filters games based on multiple criteria", () => {
      ...
      const tree = renderer
        .create(<FilteredGamesComponent filters={filters} />)
        .toJSON();
      expect(tree).toMatchSnapshot();
    });
  ...
  describe("usePaginatedGames", () => {
    it("returns correct pagination data", () => {
      const tree = renderer
        .create(<PaginatedGamesComponent page={1} pageSize={10} />)
        .toJSON();
      expect(tree).toMatchSnapshot();
    });
  });
