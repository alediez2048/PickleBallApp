...
import type { Game, Location } from "@/types/games";
...
jest.mock("../LoadingSpinner", () => ({
  LoadingSpinner: (props: { message?: string }) =>
    createMockElement("View", {
...
describe("GameList", () => {
  ...
  it("renders correctly with empty list", () => {
    ...
    const tree = renderer
      .create(<GameList data={[]} ListEmptyComponent={emptyComponent} />)
      .toJSON();
  ...
  it("renders list of games", () => {
    const games = [createMockGame("1"), createMockGame("2")];

    const tree = renderer.create(<GameList data={games} />).toJSON();

    expect(tree).toMatchSnapshot("game list with items");
  });

  it("shows loading state", () => {
    const tree = renderer.create(<GameList loading={true} />).toJSON();
  ...
  it("shows error state", () => {
    ...
    const tree = renderer.create(<GameList error={errorMessage} />).toJSON();
